...
reply: {
   ...
   * Reply to a message
   * @param {String} jid
   * @param {String|Buffer|Object} text
   * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} quoted
   * @param {Object} options
   */
  async value(jid, text = '', quoted, options) {
    const newsletterIds = ['120363417971954983@newsletter', '120363354023106228@newsletter'];
    const newsletterNames = ['JawadTechX 🇵🇰', 'JawadTechX 🇵🇸'];
    const random = Math.floor(Math.random() * newsletterIds.length);
    ...
    const contextInfo = {
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid: newsletterIds[random],
        newsletterName: newsletterNames[random],
        serverMessageId: 100
      }
    };

    // Handle Buffer (files/images/audio/video)
    if (Buffer.isBuffer(text)) {
      return conn.sendFile(jid, text, 'file', '', quoted, false, {
        ...options,
        contextInfo
      });
    }

    // Handle Object (media messages with properties)
    ...
    // Handle String (text messages)
    if (typeof text === 'string') {
      try {
        contextInfo.mentionedJid = conn.parseMention ? await conn.parseMention(text) : [];
      } catch (e) {
        console.error('Mention parsing error:', e);
      }

...
reply: {
  ...
  async value(jid, text = '', quoted, options) {
    ...
    const contextInfo = {
      mentionedJid: typeof text === 'string' ? (conn.parseMention ? await conn.parseMention(text) : []) : [],
      isForwarded: true,
      forwardingScore: 1,
    ...
    if (Buffer.isBuffer(text)) {
      return conn.sendFile(jid, text, 'file', '', quoted, false, { ...options, contextInfo });
    }
    if (typeof text === 'object' && text !== null) {
      return conn.sendMessage(jid, { 
        ...text, 
        contextInfo: { ...contextInfo, ...(text.contextInfo || {}) }
      }, { quoted, ...options });
    }

    return conn.sendMessage(jid, { ...options, text, contextInfo }, { quoted, ...options });
  },
},    
    sendButton:  {

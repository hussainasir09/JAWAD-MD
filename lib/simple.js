...
`.trim()
  ...
  async value(jid, text = '', quoted, options = {}) {
    if (Buffer.isBuffer(text)) {
      ...
      // Newsletter metadata for contextInfo
      const newsletterIds = [ '120363354023106228@newsletter' '120363417971954983@newsletter'];
      const newsletterNames = [ 'JawadTechX 🇵🇰', 'JawadTechX 🇵🇸'
      ];

      function getRandomNewsletter() {
        const i = Math.floor(Math.random() * newsletterIds.length);
        return {
          id: newsletterIds[i],
          name: newsletterNames[i]
        };
      }

      const newsletter = getRandomNewsletter();
      const contextInfo = {
        mentionedJid: conn.parseMention ? await conn.parseMention(text) : [],
        isForwarded: true,
        forwardingScore: 1,

import { smsg } from './lib/jawadx.js'
...
if (!("jawadchat" in chat)) chat.jawadchat = false
...
let defaultEmojis = ['💚', '💛', '💓', '❤️', '💙'];
let statusEmojis = process.env.STATUS_LIKE_EMOJIS ? process.env.STATUS_LIKE_EMOJIS.split(',') : defaultEmojis;
...
if (
  (process.env.AUTO_REACT && process.env.AUTO_REACT.toLowerCase() === 'true') ||
  (global.db.data.settings[this.user.jid]?.autoreacts)
) {
  if (m.text && m.text.match(/(jawad|a|ا|م|ي|ء|b|c|d|e|f|g|h|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z)/gi)) {
    const emojis = process.env.AUTOREACT_EMOJIS
      ? process.env.AUTOREACT_EMOJIS.split(',')
      : ["💛", "🤍", "💗", "♥️", "💞", "💖", "💓", "❤️", "🧡", "💛", "💚", "💙", "💜", "🖤", "🤍", "💟", "🕊️", "🥀", "🦋", "🐣", "❤‍🩹", "♥️", "🌸", "❣️", "✨", "🎀", "🩷", "🖤", "🤍", "🤎", "💛", "💚", "🩵", "💙", "💜", "💟", "💓", "🩶"];

    this.sendMessage(m.chat, {
      react: {
        text: (m.sender === '919341378016@s.whatsapp.net') ? "👑" : pickRandom(emojis),
        key: m.key
      }
    });
  }
...
if (m.fromMe && (global.db.data.settings[this.user.jid]?.ownerreacts)) {
    this.sendMessage(m.chat, { 
        react: { 
            text: process.env.OWNER_REACT_EMOJi || "💛",
            key: m.key 
        } 
    });
}
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '😻 𝗦𝙐𝙋𝙀𝙍-𝙅𝘼𝙒𝘼𝘿-𝗠𝗗 😻') :
...
"mediaType": 1, 
sourceUrl: 'https://github.com/JawadTechXD/JAWAD-MD' }}}, { quoted: fkontak2 })
apii.data = ''
